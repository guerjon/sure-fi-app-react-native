...
public class Peripheral extends BluetoothGattCallback {
	...
	*/
	...
	public void controllHVACConnect(Callback callback,Activity activity,byte[] data,int device_type,int device_id,String tx){
	...
	public void writesecurityStringForNewModuleAndHVAC(){
		...
		Log.d(METHOD_TAG,"writesecurityStringForHVAC() " + showByteArrayLikeHex(ADV_DATA_CHAR_SHORT_UUID_VALUE,true));
	...
	public void onServicesDiscovered(BluetoothGatt gatt, int status) {
		...
		if(this.first_connecting){
			...
			if(device_type == HVAC_EQUIPMENT || device_type == HVAC_THERMOSTAT){
				readADVCharacteristic();
			}else if(device_type == MODULE_WIEGAND_CENTRAL || device_type == MODULE_WIEGAND_REMOTE){
				readADVCharacteristic();
			}
			else{
				writeSecurityString();
	...
	* */
	public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) { // status is the result of the operation and new state is the new status with respect to the bridge
		...
		if (newState == BluetoothGatt.STATE_CONNECTED) {
			...
			gatt.discoverServices(); //always discover services before do anything at least once
	...
	public byte[] getModuleCentralEquipmentCommands(){
		Log.d(LOG_TAG,"getModuleEquipmentCommands()");
		byte[] commands = {
				Commands.PhoneCmd_GetDemoModeTime,
				Commands.PhoneCmd_GetAppVersion,
				Commands.PhoneCmd_GetBluetoothVersion,
	...
	public void getAllInfo() {
		...
		if(device_type == MODULE_WIEGAND_CENTRAL){
			commands = getModuleCentralEquipmentCommands();
		}

		if(device_type == MODULE_WIEGAND_REMOTE) {
