...
public class Peripheral extends BluetoothGattCallback {
	...
	public void disconnect() {
		...
		connected = false;
		Log.d("Disconnect","The method discconect is been executed");
		if (gatt != null) {
			try {
				Log.d("DEBUG","------");
				gatt.disconnect();
				...
				Log.d("EXCEPTION_ON_DISCONNECT",e.toString());
				sendConnectionEvent(device, "BleManagerDisconnectPeripheral");
	...
	public void specialWrite(UUID serviceUUID, UUID characteristicUUID, byte[] data, Integer maxByteSize, Integer queueSleepTime, Callback callback, int writeType) {
		if (gatt == null) {
			Log.e("ERROR","BluetoothGatt is null");
		} else {
			BluetoothGattService service = gatt.getService(serviceUUID);
			if(service == null)
				return;
				...
				if (writeQueue.size() == 0 && writeCallback == null) {
					...
					if (data.length > maxByteSize) {
						...
						try{
							...
							Log.d("ERROR_ON_SPECIAL_WRITE",e.toString());
						}
	...
	public void unPair(UUID serviceUUID, UUID characteristicUUID, Integer maxByteSize, Integer queueSleepTime, Callback callback, int writeType){
		if (gatt == null) {
			...
			if (characteristic == null) {
				...
				if ( writeCallback != null) {
					callback.invoke("You're already writing 3");
				}
	...
	public void write(UUID serviceUUID, UUID characteristicUUID, byte[] data, Integer maxByteSize, Integer queueSleepTime, Callback callback, int writeType) {
		if (gatt == null) {
			...
			if (characteristic == null) {
				...
				if ( writeCallback != null) {
					//write(serviceUUID,characteristicUUID,data,maxByteSize,queueSleepTime,callback,writeType);
					Log.d("Crazy","You're already writing 1");
					//callback.invoke("You're already writing 1");
					writeCallback = null;
				}
				...
				if (writeQueue.size() == 0 && writeCallback == null) {
					...
					if (data.length > maxByteSize) {
						...
						if (gatt.writeCharacteristic(characteristic)) {
							Log.d("GATT_WRITE_CHARAC","success");
							if (BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE == writeType) {
								callback.invoke();
