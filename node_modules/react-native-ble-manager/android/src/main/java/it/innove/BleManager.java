...
class BleManager extends ReactContextBaseJavaModule implements ActivityEventListener {
	...
	@ReactMethod
	public void controllHVACConnect(String peripheralUUID,ReadableArray data,int type,int device_type,int device_id,String tx,Callback callback){
		Log.d("METHOD","controllHVACConnect");
		Log.d(LOG_TAG, peripheralUUID + " " + type  + " " + device_type + " "+ device_id + " " + tx);
		Peripheral peripheral = addPeripherial(peripheralUUID);
		byte[]decoded = decodeReadableArrayToByteArray(data);
		peripheral.controllHVACConnect(callback, getCurrentActivity(),decoded,type,device_type,device_id,tx);
	}
