...
class BleManager extends ReactContextBaseJavaModule implements ActivityEventListener {
	...
	public void simpleConnect(String peripheralUUID, Callback callback){
		...
		peripheral.simpleConnect(callback, getCurrentActivity());

	}


	@ReactMethod
	public void controllConnect(String peripheralUUID,ReadableArray data,int type,Callback callback){
		Log.d("METHOD","controllConnect()");
		Peripheral peripheral = peripherals.get(peripheralUUID);
		...
		peripheral.controllConnect(callback, getCurrentActivity(),decoded,type);
	}
	...
	public void unPair(String deviceUUID, String serviceUUID, String characteristicUUID, Integer maxByteSize, Callback callback){
		...
		byte[] bytes = {0,0,0};
		if (peripheral != null){
			peripheral.write(UUIDHelper.uuidFromString(serviceUUID), UUIDHelper.uuidFromString(characteristicUUID), bytes, maxByteSize, null, callback, BluetoothGattCharacteristic.WRITE_TYPE_DEFAULT);
		} else
	...
	public  void specialWrite(String deviceUUID, String serviceUUID, String characteristicUUID, ReadableArray message, Integer maxByteSize){
		...
		for (int i = 0; i < message.size(); i++) {
			decoded[i] = new Integer(message.getInt(i)).byteValue();
			int positive = decoded[i] & 0xff;
			//Log.d("byte",String.valueOf(positive));
		}

		Log.d(LOG_TAG, "Message2(" + decoded.length + "): " + bytesToHex(decoded));
	...
	public void specialWriteWithoutResponse(String deviceUUID, String serviceUUID, String characteristicUUID, ReadableArray message, Integer maxByteSize, Integer queueSleepTime) {
		...
		if (peripheral != null){
			...
			Log.d(LOG_TAG, "Message(" + decoded.length + "): " + bytesToHex(decoded));
			peripheral.specialWrite(UUIDHelper.uuidFromString(serviceUUID), UUIDHelper.uuidFromString(characteristicUUID), decoded, maxByteSize, queueSleepTime, callback, BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE);
		} else
			callback.invoke("Peripheral not found");
	}
