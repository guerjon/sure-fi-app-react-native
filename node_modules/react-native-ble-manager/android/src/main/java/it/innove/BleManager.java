...
class BleManager extends ReactContextBaseJavaModule implements ActivityEventListener {
	...
	private Peripheral current_peripherial = null;
	private String write_type = "write";
	...
	private BluetoothAdapter getBluetoothAdapter() {
		Log.d(LOG_TAG, "getBluetoothAdapter()");
	...
	@ReactMethod
	public void getHVACDeviceStatus(String peripheralUUID, Callback callback){
		Peripheral peripheral = peripherals.get(peripheralUUID);
		callback.invoke(String.valueOf(peripheral.getHVACDeviceStatus()));
	}

	private Peripheral addPeripherial(String peripheralUUID){
		Log.d(LOG_TAG, "addPeripherial()");
		Peripheral peripheral = peripherals.get(peripheralUUID);
		if (peripheral == null) {
			...
			if (BluetoothAdapter.checkBluetoothAddress(peripheralUUID)) {
				...
				Log.d("METHOD","Invalid peripheral uuid");
			}
		}
		return peripheral;
	}
	...
	public void connect(String peripheralUUID, Callback callback) {
		...
		Peripheral peripheral = this.addPeripherial(peripheralUUID);
		peripheral.connect(callback, getCurrentActivity(),false,"disconnected");
	}

	@ReactMethod
	public void simpleConnect(String peripheralUUID, Callback callback){
		Log.d(LOG_TAG, "SimpleConnect(): " + peripheralUUID );
		simple_connect = true;
		Peripheral peripheral = addPeripherial(peripheralUUID);
		peripheral.connect(callback, getCurrentActivity(),simple_connect,"disconnected");
	}

	@ReactMethod
	public void controllConnect(String peripheralUUID,ReadableArray data,int type,int device_type,Callback callback){
		Log.d(LOG_TAG, "controllConnect(): " );
		Peripheral peripheral = this.addPeripherial(peripheralUUID);
		byte[] decoded = decodeReadableArrayToByteArray(data);
		peripheral.controllConnect(callback, getCurrentActivity(),decoded,type,device_type);
	}

	@ReactMethod
	public void controllHVACConnect(String peripheralUUID,ReadableArray data,int device_type,int device_id,String tx,Callback callback){
		Log.d("METHOD","controllHVACConnect");
		Log.d(LOG_TAG, peripheralUUID + "  " + device_type + " "+ device_id + " " + tx);
		Peripheral peripheral = addPeripherial(peripheralUUID);
		byte[]decoded = decodeReadableArrayToByteArray(data);
		peripheral.controllHVACConnect(callback, getCurrentActivity(),decoded,device_type,device_id,tx);
	}

	@ReactMethod
	public void connectWithoutResponse(String peripheralUUID){
	...
	@ReactMethod public void getAllInfo(String peripheralUUID){
		Peripheral peripheral = peripherals.get(peripheralUUID);
		peripheral.getAllInfo();
	}

	@ReactMethod public void getSomeInfo(String peripheralUUID,ReadableArray commands){
		Log.d(LOG_TAG, "getSomeInfo() 1");
		Peripheral peripheral = peripherals.get(peripheralUUID);
		byte[] data = decodeReadableArrayToByteArray(commands);
		peripheral.getSomeInfo(data);
	}
	...
	@ReactMethod
	public void startHVACNotifications(String deviceUUID){
		Peripheral peripheral = peripherals.get(deviceUUID);
		if(peripheral != null){
			peripheral.startHVACNotifications();
		}
	}
	...
	private void doWrite(String deviceUUID,ReadableArray data,String service_uuid,String characteristic_uuid,Callback callback,int writeType){
		Peripheral peripheral = peripherals.get(deviceUUID);

		if (peripheral != null){
			byte[] decoded = decodeReadableArrayToByteArray(data);

			peripheral.write(UUIDHelper.uuidFromString(service_uuid),UUIDHelper.uuidFromString(characteristic_uuid), decoded, callback, writeType);
		} else {
			callback.invoke("Peripheral not found");
		}
	}
	...
	public void write(String deviceUUID, String serviceUUID, String characteristicUUID, ReadableArray message , Integer maxByteSize, Callback callback) {
		doWrite(deviceUUID,message,serviceUUID,characteristicUUID,callback,BluetoothGattCharacteristic.WRITE_TYPE_DEFAULT);
	}

	@ReactMethod
	public void unPair(String deviceUUID, String serviceUUID, String characteristicUUID, Integer maxByteSize, Callback callback){
		Log.d(LOG_TAG, "Write to unpair on device: " + deviceUUID);
		Peripheral peripheral = peripherals.get(deviceUUID);
		byte[] bytes = {0,0,0};
		if (peripheral != null){

			peripheral.write(UUIDHelper.uuidFromString(serviceUUID), UUIDHelper.uuidFromString(characteristicUUID), bytes, callback, BluetoothGattCharacteristic.WRITE_TYPE_DEFAULT);
		} else
			Log.d("ERROR","Peripheral not found");
	}

	@ReactMethod
	public  void specialWrite(String deviceUUID, String serviceUUID, String characteristicUUID, ReadableArray message, Integer maxByteSize){
		doWrite(deviceUUID,message,serviceUUID,characteristicUUID,null,BluetoothGattCharacteristic.WRITE_TYPE_DEFAULT);
	}

	@ReactMethod
	public void specialWriteWithoutResponse(String deviceUUID, String serviceUUID, String characteristicUUID, ReadableArray message, Integer maxByteSize, Integer queueSleepTime) {
		doWrite(deviceUUID,message,serviceUUID,characteristicUUID,null,BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE);
	}


	@ReactMethod
	public void initPeripheral(String deviceUUID){
		current_peripherial = peripherals.get(deviceUUID);
	}

	@ReactMethod
	public void fastWrite(String service_uuid,String characteristic_uuid,ReadableArray data){
		if (current_peripherial != null){
			byte[] decoded = decodeReadableArrayToByteArray(data);
			current_peripherial.write(UUIDHelper.uuidFromString(service_uuid),UUIDHelper.uuidFromString(characteristic_uuid), decoded, null, BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE);
		} else {
			Log.d(LOG_TAG, "Error on fast write");
		}
	}
	...
	public void retrieveServices(String deviceUUID, Callback callback) {
		Log.d(LOG_TAG, "retrieveServices()");
		Peripheral peripheral = peripherals.get(deviceUUID);
		if (peripheral != null){
				...
				public void onLeScan(final BluetoothDevice device, final int rssi,
					...
					runOnUiThread(new Runnable() {
						...
						public void run() {
							...
							Log.i(METHOD_TAG, "scanRecord: " + scanRecord);
							Log.i(METHOD_TAG, "DiscoverPeripheral: " + device.getName());
	...
	@ReactMethod
	public  void transformImageToBinaryData(String uri, Callback callback){
		try {
			...
			if (image == null) {
				...
				String s = new String(bitmapToBase64(image));
				callback.invoke(s);
			}
		} catch (IOException e) {
		}
	...
	private byte[] decodeReadableArrayToByteArray(ReadableArray data){
