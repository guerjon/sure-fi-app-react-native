...
class SetupCentral extends Component{
    ...
    writeUnpairForEquipment(){
    	HVAC_WRITE_COMMAND(this.device.id,[0x46,0])
    }

    writeUnpairForThermostat(uuids){
    	var data = [0x46,0]
    	HVAC_WRITE_COMMAND(this.device.id,data.concat(uuids))
    }
	...
	writePairResult(device){
		...
		HVAC_WRITE_COMMAND(device.id,[0x21])
		.then(response => {
			//this.setConnectionEstablished(device)
		})
	...
	writeUnpairResult(device){
		...
		HVAC_WRITE_COMMAND(device.id,[0x22])
		.then(response => {
			//this.setConnectionEstablished(device)
		})
	...
	setDebugModeStatus(){
		if(this.props.debug_mode_status){ // debug mode is enabled
			...
			HVAC_WRITE_COMMAND(this.device.id,[0x28,0x00])
			.then(response => {
			...
			HVAC_WRITE_COMMAND(this.device.id,[0x28,0x01])
			.then(response => {
	...
	clearResetCauses(){
		HVAC_WRITE_COMMAND(this.device.id,[0x27])
		.then(response => {
		})
	...
	resetBoard(){
		HVAC_WRITE_COMMAND(this.device.id,[0x1C])
		.then(response => {
		})
	...
	getLastPackageTime(){
		HVAC_WRITE_COMMAND(this.device.id,[0x2A])
		.then(response => {
		})
	...
	getResetCauses(){
		...
		HVAC_WRITE_COMMAND(this.device.id,[0x26])
		.then(response => {
		})
	...
	writeRandomCommand(){
		...
		HVAC_WRITE_COMMAND(this.device.id,[item])
	...
	updateOperatingValues(values){
		console.log("updateOperatingValues", this.prettyBytesToHex(values))	
	}
	...
	prettyBytesToHex(bytes){
		var string_bytes = this.bytesToHex(bytes)
		var final_string = ""
	...
	getAllInfo(device){
		...
		if(!this.props.getting_commands){
			...
			console.log("hardware_type",this.device.manufactured_data.hardware_type)
			if(this.device.manufactured_data.hardware_type == '03'){
	...
	getFirmwareVersion(){
		...
		this.createGeneralInterval(() => {
	    	HVAC_WRITE_COMMAND(this.device.id,[PhoneCmd_GetAppVersion])
	    	.then(response => {		    		
	    	})
	    	.catch(error => console.log("error",error))						
		})
	...
	getRadioFirmwareVersion(){
		...
		this.createGeneralInterval(() => {			
			HVAC_WRITE_COMMAND(this.device.id,[PhoneCmd_GetRadioVersion])
	    	.then(response => {
			})
	...
	getBluetoothFirmwareVersion(){
		...
		this.createGeneralInterval(() => {
			HVAC_WRITE_COMMAND(this.device.id,[PhoneCmd_GetBluetoothVersion])
			.then(response => {
			})
	...
	getRegistration(){
		this.createGeneralInterval(() => {
			HVAC_WRITE_COMMAND(this.device.id,[PhoneCmd_GetRegistration])
			.then(response => {

			})
			.catch(error => console.log("error getting the registration",error))
		})
	}
	...
	getRadioSettings(){
		...
		this.createGeneralInterval(() => {
			HVAC_WRITE_COMMAND(this.device.id,[COMMAND_GET_RADIO_SETTINGS])
			.then(response => {
	...
	getVoltage(){
		...
		this.createGeneralInterval(() => {			
	    	HVAC_WRITE_COMMAND(this.device.id,[COMMAND_GET_VOLTAGE])
	    	.then(response => {
	...
	getRegisteredBoard1(){
		...
		this.createGeneralInterval(() => {
			HVAC_WRITE_COMMAND(this.device.id,[COMMAND_GET_REGISTERED_BOARD_1])
			.then(response => {
	...
	getRegisteredBoard2(){
		...
		this.createGeneralInterval(() => {
			HVAC_WRITE_COMMAND(this.device.id,[COMMAND_GET_REGISTERED_BOARD_2])
			.then(response => {
			}).catch(error => console.log("error",error))							
		})
	}
	...
	getAppPicVersion(){
		...
		this.createGeneralInterval(() => {			
			HVAC_WRITE_COMMAND(this.device.id,[COMMAND_GET_APP_PIC_VERSION])
	    	.then(response => {
			}).catch(error => console.log("error",error))			
		})
	}
	...
	getRadioPicVersion(){
		...
		this.createGeneralInterval(() => {
	    	HVAC_WRITE_COMMAND(this.device.id,[COMMAND_GET_RADIO_PIC_VERSION])
	    	.then(response => {
	    	}).catch(error => console.log("error",error))
		})
	}
	...
	getDebugModeStatus(){
		...
		this.createGeneralInterval(() => {
			HVAC_WRITE_COMMAND(this.device.id,[COMMAND_GET_DEBUG_MODE_STATUS]) // response with 0x1B
			.then(response => {
			})
	...
	getOperatingValues(){
		...
		HVAC_WRITE_COMMAND(this.device.id,[PhoneCmd_GetOperatingValues])
		.then(response => {
		})
	...
	getAllVersion(){
		...
		this.createGeneralInterval(() => {
			HVAC_WRITE_COMMAND(this.device.id,[COMMAND_GET_ALL_VERSIONS]) // should get a 0x1E	
		})
	}
	...
	getBootloaderInfo(){
		...
		this.createGeneralInterval(() => {
			HVAC_WRITE_COMMAND(this.device.id,[COMMAND_GET_BOOTLOADER_INFO]) 	
		})
	}
	...
	handleCharacteristicNotification(data){
		console.log("handleCharacteristicNotification", this.prettyBytesToHex(data.value))
	...
	handleResponse(command,data){
		console.log("handleResponse",command.toString(16) + " " +  this.prettyBytesToHex(data))
		switch(command){
			case PhoneRsp_AppVersion:
				this.updateAppVersion(data)
				this.getBluetoothFirmwareVersion()
			break
			case PhoneRsp_RadioVersion:
				this.updateRadioVersion(data)
				this.getFirmwareVersion()
			break
			case PhoneRsp_BluetoothVersion:
				this.updateBluetoothVersion(data)
				this.getRegistration()
			break
			case PhoneRsp_Registration:
				this.updateRegistration(data)
				this.getOperatingValues()
			break
			case PhoneRsp_Success:
				console.log("Success",data)
				//Alert.alert("Success");
			break
			case PhoneRsp_Failure:
				var writed_command = data[0]
				var response_code = data[1]
				this.handleFailure(writed_command,response_code)
			break
			case PhoneRsp_UartTimeout:
				Alert.alert("Failure", "PhoneRsp_UartTimeout (" + command.toString(16) + ") data: ( " + this.prettyBytesToHex(data) + ")")
			break
			default:
				console.log("No options found to: (" + command.toString(16) + ") with data: (" + this.prettyBytesToHex(data) + ")");
			break
		}
	}

	handleRead(command,data){
		console.log("handleRead(",command + " " +  this.prettyBytesToHex(data) +")")
		switch(command){
			case PhoneCmd_GetRegistration:
				this.updateRegistration(data)
				this.getOperatingValues()
			break
			case PhoneRsp_OperatingValues:
