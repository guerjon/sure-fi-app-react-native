...
class ScanCentralUnits extends Component {
  ...
  componentDidMount() {
    ...
    dispatch({type: "CONFIGURATION_RESET_CENTRAL_REDUCER"})
  ...
  */
  ...
  onSuccess(scan_result) {
    ...
    //console.log("matched device",matched_device)
    ...
    if(matched_device.length > 0){
        ...
        if(flag){
          if(( state== '0203') ||  (state == '0003')){
            dispatch({type: "CONFIGURATION_CENTRAL_DEVICE_MATCHED",central_device : matched_device[0]});
          }else{
            dispatch({type: "CONFIGURATION_CENTRAL_DEVICE_IS_NOT_ON_PAIRING_MODE" })
          }
      ...
      dispatch({type: "CONFIGURATION_CENTRAL_DEVICE_NOT_MATCHED"})
    }
  ...
  resetQrState(){
    var {dispatch} = this.props;
    dispatch({type: constants.CONFIGURATION_RESET_CENTRAL_REDUCER})
  }
...
const mapStateToProps = state => ({
  ...
  scanned_central_units : state.configurationScanCentralReducer.scanned_central_units,
  central_device_matched : state.configurationScanCentralReducer.central_device_matched,
  central_device: state.configurationScanCentralReducer.central_device,
  bluetooth_error : state.configurationScanCentralReducer.bluetooth_error,
  manufactured_data : state.configurationScanCentralReducer.manufactured_data,
  central_device_is_not_on_pairing_mode : state.configurationScanCentralReducer.central_device_is_not_on_pairing_mode
})
